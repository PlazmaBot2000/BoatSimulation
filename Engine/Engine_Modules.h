...
public:
    ...
    if (scaleMode == ScaleMode::Repeat) {
        ...
        SDL_RenderCopyEx(renderer, target, NULL, &dest, collider.angle, NULL, SDL_FLIP_NONE);
        
        SDL_DestroyTexture(target);
    } else {
        SDL_Rect dest = { (int)collider.x, (int)collider.y, (int)collider.width, (int)collider.height };
        SDL_RenderCopyEx(renderer, texture, NULL, &dest, collider.angle, NULL, SDL_FLIP_NONE);
    }
